...
public class LemonFoliagePlacer extends FoliagePlacer {
    public static final Codec<LemonFoliagePlacer> CODEC = RecordCodecBuilder.create(instance ->
            foliagePlacerParts(instance).and(Codec.intRange(0,16).fieldOf("height").forGetter(fp -> fp.height))
                    .apply(instance, LemonFoliagePlacer::new));

    private final int height;

    public LemonFoliagePlacer(IntProvider pRadius, IntProvider pOffset, int height) {
    ...
    protected FoliagePlacerType<?> type() {
        return EPFoliagePlacers.LEMON_FOLIAGE_PLACER.get();
    }
    ...
    protected void createFoliage(LevelSimulatedReader pLevel, FoliageSetter pSetter, RandomSource pRandom,
        ...
        this.placeLeavesRow(pLevel, pSetter, pRandom, pConfig, pAttachment.pos().above(-2),3,0, pAttachment.doubleTrunk());
        ...
        this.placeLeavesRow(pLevel, pSetter, pRandom, pConfig, pAttachment.pos().above(3),0,0, pAttachment.doubleTrunk());
    }
