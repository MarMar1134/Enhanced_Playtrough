package net.marmar.enhanced_playthrough.data.loot.entity;
...
public class EPEntityLootTables implements LootTableSubProvider {
    public static final ResourceLocation ZOMBIE_KNIGHT = register("zombie_knight");
    public static final ResourceLocation SKELETON_BOWMASTER = register("skeleton_bowmaster");
    public static final ResourceLocation WERELLAGER = register("werellager");

    @Override
    public void generate(BiConsumer<ResourceLocation, LootTable.Builder> pOutput) {
        pOutput.accept(ZOMBIE_KNIGHT, zombieKnightLootTable());
        pOutput.accept(SKELETON_BOWMASTER, skeletonBowmasterLootTable());
        pOutput.accept(WERELLAGER, werellagerLootTable());
    }
    ...
    public static LootTable.Builder werellagerLootTable(){
        return LootTable.lootTable()
                .withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1))
                        .add(LootItem.lootTableItem(EPItems.WERELLAGER_HIDE.get())
                                .when(LootItemRandomChanceWithLootingCondition.randomChanceAndLootingBoost(0.3f, 0.2f))
                                .apply(SetItemCountFunction.setCount(UniformGenerator.between(0,2)))
                                .apply(LootingEnchantFunction.lootingMultiplier(UniformGenerator.between(0,1))))
                )
                .withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1))
