...
public class EPDamageSources {
    ...
    public EPDamageSources(RegistryAccess registryAccess){
        this.damageTypes = registryAccess.registryOrThrow(Registries.DAMAGE_TYPE);

        this.tall_reed = this.source(EPDamageTypes.TALL_REED);
        this.cobble = this.source(EPDamageTypes.COBBLE);
        this.bleed = this.source(EPDamageTypes.BLEED);
    }
